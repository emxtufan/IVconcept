...
import { motion, useScroll, useTransform, useSpring, useReducedMotion, type MotionStyle } from 'motion/react';
import { getSiteContent } from '../data';
import TrueFocus from './TrueFocus';

export default function CommitmentSection() {
  const reducedMotion = useReducedMotion();
  const revealStyle = (style: MotionStyle): MotionStyle => reducedMotion ? { opacity: 1, x: 0, y: 0, scale: 1, scaleX: 1 } : style;
  ...
  return (
    ...
    >
      ...
      />
      <motion.div
        style={revealStyle({ x: blX, y: blY, opacity: cornerOpacity })}
      ...
      />
      <motion.div
        style={revealStyle({ x: brX, y: brY, opacity: cornerOpacity })}
      ...
      <div className="mx-auto flex w-full max-w-[1140px] flex-col items-center justify-center py-12 text-center md:py-20">
        <motion.span
          style={revealStyle({ scale: labelScale, y: labelY, opacity: labelOpacity })}
        ...
        {showLogo ? (
          <motion.div
            style={revealStyle({ scale: logoScale, y: logoY, opacity: logoOpacity })}
          ...
          <div className="flex w-full flex-col items-center pt-6 md:pt-10">
            <motion.h2
              style={revealStyle({ scale: headlineScale, y: headlineY, opacity: headlineOpacity })}
            ...
            <motion.div
              style={revealStyle({ scaleX: dividerScaleWidth, opacity: dividerOpacity })}
              className="my-4 h-px w-12 bg-[#c5a880]/20"
            />

            <motion.p
              style={revealStyle({ scale: paragraphScale, y: paragraphY, opacity: paragraphOpacity })}
