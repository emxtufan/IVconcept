...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const pointers: PointerItem[] = [new PointerItem()];
    ...
    const displayShaderSource = `
      ...
      void main () {
          ...
          // Compute premium cover alignment coordinates for the background image
          vec2 uv = (vUv - 0.5) * uScale + 0.5;
          vec3 bg = texture2D(uBgImage, uv).rgb;
          ...
          gl_FragColor = vec4(bg, clamp(density, 0.0, 1.0));
      }
    `;
    ...
    function drawDisplay(target: any) {
      ...
      // Bind cover alignment uBgImage
      gl.activeTexture(gl.TEXTURE1);
      gl.bindTexture(gl.TEXTURE_2D, bgTexture);
      gl.uniform1i(displayMaterial.uniforms.uBgImage, 1);

      // Compute perfect background-size: cover scaling coordinates dynamically
      ...
      gl.uniform2f(displayMaterial.uniforms.uScale, scaleX, scaleY);
      
      blit(target);
    }

    function splatPointer(pointer: PointerItem) {
      const dx = pointer.deltaX * config.SPLAT_FORCE;
      const dy = pointer.deltaY * config.SPLAT_FORCE;
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy);
    }

    function clickSplat(pointer: PointerItem) {
    ...
    function splat(x: number, y: number, dx: number, dy: number) {
      ...
      // Always splat pure white (1, 1, 1) as density, to act as perfect reveal mask!
    ...
    function updatePointerDownData(pointer: PointerItem, id: number, posX: number, posY: number) {
      pointer.id = id;
      pointer.down = true;
      pointer.moved = false;
    ...
    function updatePointerMoveData(pointer: PointerItem, posX: number, posY: number) {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
      pointer.texcoordY = 1.0 - posY / canvas.height;
    ...
    function updatePointerUpData(pointer: PointerItem) {
      pointer.down = false;
    }
